...
func GetChunk(chunkList string, pos int) int {
...
func GetLowestRepRateIndex(bandwithList []int) int {
	lowestindex := 0
	for i := 0; i < len(bandwithList); i++ {
		if bandwithList[i] < bandwithList[lowestindex] {
			lowestindex = i
		}
	}
